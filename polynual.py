...
def MUL_PQ_P (p1,x):
    """
    умножает многочлен р1 в нормальном виде (при старшем коэффициенте стоит не ноль) на число х типа RationalNumber
    """
    p3=copy.deepcopy(p1)
...
def LED_P_Q(p1):
    """
    возвращает старший коэффициент в формате RationalNumber многочлена р1 в нормальном виде
    """
    p1r=copy.deepcopy(p1)
...
def FAC_P_Q(p1):
    ...
    возвращает вынесенное число (Р) формата RationalNumber и многочлен р3
    """
    p3=Polynom([])
    p1=copy.deepcopy(p1)
    p2=Polynom([])
